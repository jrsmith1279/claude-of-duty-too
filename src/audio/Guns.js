...
export const SIGNATURES = {
  ...
  mp5: {
    level: 0.62, crackF: 2450, crackQ: 1.3, crackDecay: 0.019, drive: 3.4,
    barkF: 880, barkQ: 1.1, barkDecay: 0.052, barkLevel: 0.40,
    bodyF0: 260, bodyF1: 88, bodyDecay: 0.042, bodyLevel: 0.34,
  ...
  m870: {
    ...
    barkF: 300, barkQ: 0.6, barkDecay: 0.175, barkLevel: 0.80,
    bodyF0: 140, bodyF1: 38, bodyDecay: 0.150, bodyLevel: 0.85,
    subLevel: 0.62, hiss: 0.40, hissDecay: 0.30,
...
export function distantReport(g, sig, o, dist, t0) {
  ...
  const crack = g.spatial(o.x || 0, o.y || 1.5, o.z || 0, {
    bus: 'weapons', gain: 1.1, ref: 60, rolloff: 0.5, airRef: 220,
    send: 0.10, noDelay: true, delay: Math.max(0, dist / 343 - lead),
  });
  if (crack && dist > 40) {
    const t = crack.when;
    const src = noiseSource(ac, 'white', 1, rnd);
    const bp = biquad(ac, 'bandpass', 2900, 0.8);
    const shaper = ac.createWaveShaper();
    shaper.curve = driveCurve(ac, 3.2);
    const env = gain(ac, 0);
    src.connect(bp); bp.connect(shaper); shaper.connect(env); env.connect(crack.input);
    hit(env.gain, t, 0.55 * (1 - far * 0.45), 0.0006, 0.016);
    fire(src, t, 0.04, () => { src.disconnect(); bp.disconnect(); shaper.disconnect(); env.disconnect(); });
    scheduleRelease(ac, crack, 0.25);
  } else if (crack) {
    crack.release();
  }
}
