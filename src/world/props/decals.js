...
function drawCell(g, name, rnd) {
  ...
  switch (name) {
    ...
    case 'wheelPath': {
      ...
      // A tone, not a mark: peak alpha 0.42 with both edges ramping all the
      // way to zero. This only works because the caller puts it in the blended
      // set — at the hard set's alphaTest of 0.45 every pixel of it is
      // discarded and the wheel path simply does not exist, which is what it
      // did until the atlas was dumped and looked at.
      ...
      grd.addColorStop(0.3, rgba(150, 146, 138, 0.42));
      grd.addColorStop(0.7, rgba(150, 146, 138, 0.42));
      ...
      for (let i = 0; i < 22; i++) {
        ...
        g.fillStyle = rgba(168, 163, 152, 0.18 + rnd() * 0.2);
...
export function tyreTracks(ctx, site, bs, kit, rand, density = 1, env = null) {  // hard set
  ...
  let count = 0;
  count += wheelPolish(site, env?.decalSoft || bs, kit, rand, density);
...
function wheelPolish(site, hard, kit, rand, density) {
  // `hard` is a misnomer here and it is deliberate: the caller hands this the
  // SOFT set. Everything else on the road goes in the depth-writing set on
  // principle, but a wheel path is the one road mark that is a *tone* rather
  // than a mark — it has no edge at all, it just fades into the tarmac either
  // side. Alpha-tested at 0.45 it becomes a 0.5 m painted stripe, which is
  // exactly what it looked like in the first version of this frame. 56 blended
  // quads against the 8700 already in that set is a rounding error, and it is
  // the one place in this file worth spending them.
  const ax = carriagewayAxis(site);
  if (!ax) return 0;
